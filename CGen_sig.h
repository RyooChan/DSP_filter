...
public:
	CGen_sig(void);
	~CGen_sig(void);
	void Gen_signal(CString Nsignal, int m_frq);

	double Sin_Data[DATA_no], Cos_Data[DATA_no], Unit_Data[DATA_no], WhiteNoise[DATA_no], SinWN[DATA_no], CosWN[DATA_no], sinc[DATA_no];
	double Ramp[DATA_no], step[DATA_no], impulse[DATA_no];
